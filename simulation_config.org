...
#+BEGIN_SRC sh :tangle simulation_header.conf
  ...
  #@description The main wall calorimeter resolution
  analysis.calo_resolution : real = 0.08 #%
...
**** BiPo visualization
#+BEGIN_SRC sh :noweb yes
  <<if_module("search_bipo_events", cut="bkg_Bi214_Po214_cut", then="io_output_tracker_trajectory_selected", else="io_output_tracker_trajectory_non_selected")>>
#+END_SRC
...
=basic_plot= module for example). This module can not be as generic as
processing module. Basically, for one study purpose, there is one or two plot
modules.
...
#+BEGIN_SRC sh
  ...
  cuts.configuration_files : string[6] =                                                  \
    ...
    "${SNAILWARE_SIMULATION_DIR}/configuration/current/event_topology_cuts.conf"
#+END_SRC
...
:PROPERTIES:
:CUSTOM_ID: event_topology_cuts
:TANGLE: event_topology_cuts.conf
:END:

All these cuts are based on a very specific cuts working on dedicated event
topology.
...
**** BiPo selection
#+BEGIN_SRC sh
  [name="bipo_selection" type="snemo::analysis::cut::special_topology_cut"]
...
**** Bremsthralhung selection
#+BEGIN_SRC sh
  [name="bremsthralhung_selection" type="snemo::analysis::cut::special_topology_cut"]

  #@description Debug flag
  debug : boolean = 0

  #@description The running mode
  mode : string = "bremsstrahlung"
#+END_SRC

**** Looking for ^214Bi events
#+CAPTION: Multi-selection for selecting ^214Bi events
#+TBLNAME: bkg_Bi214_Po214_cuts :results none
|-----------------------------|
| has_tracker_trajectory_data |
| bipo_selection              |
|-----------------------------|
...
#+BEGIN_SRC sh :noweb yes
  <<multi("bkg_Bi214_Po214_cut", "and", 2, bkg_Bi214_Po214_cuts)>>
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  #@description The X axis label
  display.xaxis.label : string = \
      "$\Upsigma_\text{\tiny 1,2}\text{E}_\text{calibrated}\geq\text{E}_\text{cut}$"
...
#+BEGIN_SRC sh
  ...
  max : real = 3200.0
#+END_SRC
**** BiPo alpha track template
#+BEGIN_SRC sh
  [name="BiPo::time_delay" type="mygsl::histogram_1d"]
  #@config The histogram of the alpha time delay

  #@description The title of the histogram (optional)
  title : string = ""

  # #@description The group of the histogram (optional)
  # group : string = "bipo::time_delay"
  ...
  #@description The X axis label
  display.xaxis.label : string = "time delay"

  #@description The X axis unit for display (a standard unit, typically SI or CLHEP)
  display.xaxis.unit : string = "us"

  #@description The Y axis label
  display.yaxis.label : string = "dN/dt [A.U.]"

...
module manager file. Loading external libraries such as =libFalaise=, we must
pass the location of such libraries using the dll configuration done in section
[[#sec::dll]]. We also have to define where the resource files are using the
=--datatools:resource_path= command line. For example, if you only want to do
simulation, you can run the following command
...
#+BEGIN_SRC sh
  bxdpp_processing                                                                                      \
      ...
      --datatools::resource_path@<where your falaise setup files are>                                   \
...
#+END_SRC
where file =/tmp/$USER/io_output_simulated.brio= is the output file produced by
the =simulation_chain= module as defined and setup [[Simulation output][in this section]].

All the different process steps can then be run using the dedicated module such
as [[Clustering algorithms][clustering]] or [[Fitting algorithm][fitting]] Geiger tracks. The most complete mode is the

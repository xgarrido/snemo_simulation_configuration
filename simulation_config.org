...
- select events with a given set of topology measurement which are :
  - select events with good internal probability :
    #+BEGIN_SRC shell
      ...
      #@description Cut description
      cut.description : string = "Select events with good internal probability"

      #@description Logging priority'
      logging.priority : string = @variant(core:logging_priority|"trace")
  ...
  - select events with bad external probability :
    #+BEGIN_SRC shell
      ...
      range_external_probability.max : real as fraction = 1e-5 %
    #+END_SRC
